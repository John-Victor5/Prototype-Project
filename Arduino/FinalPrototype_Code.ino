...
int stepDelay = 7000;  // Microseconds between steps
bool usedms = true;
int stepDelayms = 2;   // CHANGED TO 2ms: 1ms is usually too fast and causes stalling/vibration
...
// =============================================================================
// ==================== NEW VARIABLE FOR AI STATE ====================
bool aiIsBusy = false; // When true, the door will NOT close even if no one is there
// ===================================================================
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (cmd == 'B') { // 'B' for BUSY (AI is generating or speaking)
      aiIsBusy = true;
    } 
    else if (cmd == 'I') { // 'I' for IDLE (AI is done speaking)
      aiIsBusy = false;
    }
  }

  // --- 2. RFID SCANNING (Keep your existing code) ---
  uint8_t atqa[2] = {0};
  uint8_t uid[4]  = {0};
  if (rc522_request(atqa)) {
    if (rc522_anticoll(uid)) {
       // Optional: Add logic here to open door on RFID if you wish
    }
    rc522_halt();
    delay(500);
  }
  ...
  // --- 4. DOOR CONTROL LOGIC ---
  
  // OPEN: If person detected for 3 seconds
  if (objectPresent) {
    if (!doorOpen && detectionStartTime != 0 && (millis() - detectionStartTime >= 3000)) {
      openDoor();
    }
  } 
  // CLOSE: If no person AND AI is not busy
  else {
    if (doorOpen && !aiIsBusy && noDetectionStartTime != 0 && (millis() - noDetectionStartTime >= 3000)) {
      closeDoor();
    }
    // If AI is busy, we reset the noDetectionStartTime so it doesn't "jump" shut 
    // immediately after the AI finishes speaking.
    if (aiIsBusy) {
      noDetectionStartTime = millis(); 
    }
  }
...
// ----------------------------------------------------------------------
// Helper function to move both servos slowly at the same time
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Helper functions to move the servos and stepper
// ----------------------------------------------------------------------
void openDoor() {
  if (!doorOpen) {
    ...
    Serial.println(">>> Door OPENED after 3s detection <<<");
  }
}
...
void closeDoor() {
  if (doorOpen) {
    ...
    Serial.println(">>> Door CLOSED after 3s no detection <<<");
  }
}
...
void rotateSteps(int steps, int direction) {
  ...
  // FIXED: Always release the motor after moving to prevent voltage starvation for the servos!
  releaseMotor(); 
}
...
// ============================================================
//  SPI Low-Level: Write / Read Register
// ============================================================
// FIXED: Wrap communications inside proper SPI transactions to free up resources when idle
...
// ============================================================
//  Transceive: send bytes, receive response
// ============================================================
...
// ============================================================
//  REQA — Request Type A (detect card presence)
// ============================================================
